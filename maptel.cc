...
namespace {
    ...
    void checkTelNumSize(string& TelNum) {
...
namespace jnp1 {
    ...
    [[maybe_unused]] void maptel_insert(MapIdType id, char const *tel_src, char const *tel_dst) {
        if(debug) {
    ...
    [[maybe_unused]] void maptel_erase(MapIdType id, char const *tel_src) {
        if(debug) {
            errMessageErase(id, CALL," ", tel_src ? tel_src : "");
        }
    ...
    [[maybe_unused]] void maptel_transform(MapIdType id, char const *tel_src, char *tel_dst,
                          size_t len) {
        if(debug) {
            errMessageTransformCall(id, tel_src ? tel_src : "", tel_dst, len);
        }
        ...
        if(debug) {
            errMessageTransformPerform(telSrcStart, tempTelSrc, cycleDetected);
        }
    }
